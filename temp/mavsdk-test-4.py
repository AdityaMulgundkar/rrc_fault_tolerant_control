...
async def run():
    drone = System()
    await drone.connect(system_address="udp://:14554")
...
async def observe_shell(drone):
    async for output in drone.shell.receive():
        # Write function to extract rpy
        # Pass values to rotation forest sdk
        print(f"\n{output} ", end='', flush=True)

def got_stdin_data(drone):
    # asyncio.ensure_future(send(drone, sys.stdin.readline()))
    asyncio.ensure_future(send(drone, "listener vehicle_rates_setpoint"))
